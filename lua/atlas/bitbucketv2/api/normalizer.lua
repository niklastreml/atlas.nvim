...
function M.normalize_pr_diffstat(result)
	local entries = {}
...
function M.normalize_pr_commits(result)
	local entries = {}
	for _, item in ipairs((result and result.values) or {}) do
...
function M.normalize_repository_tags(result)
	local entries = {}
	for _, item in ipairs((result.values or {})) do
		local target = item.target or {}
		local author = (((target.author or {}).user or {}).nickname)
...
---@param repo string|nil
---@return BitbucketPR
function M.normalize_pr(raw_pr, workspace, repo)
	return normalize_pr(raw_pr, workspace, repo)
end
