...
function M.register(buf, views)
	...
	if state.provider and state.provider.search then
		utils.insert_if(items, item("issues.search", {
			desc = "Search issues",
			index = 2,
			callback = function()
				actions.search()
	...
	utils.insert_if(items, item("issues.copy_key", {
		desc = "Copy issue key",
		opts = { nowait = true },
		callback = function()
			local issue = selected_issue()
	...
	utils.insert_if(items, item("issues.copy_url", {
		desc = "Copy issue URL",
		opts = { nowait = true },
		callback = function()
			local issue = selected_issue()
	...
	utils.insert_if(items, item("issues.show_details", {
	...
	utils.insert_if(items, item("issues.toggle_issue_children", {
