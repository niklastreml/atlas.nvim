...
function M.cell_hl(row, col, ctx)
	...
	if col.key == "pr_icon" then
		return {
			{ start_col = 0, end_col = #ctx.padded, hl_group = "AtlasTextPositive" },
		}
	end
	...
	if col.key == "author" then
		local hl_group = highlights.dynamic_for(row.author)
		return {
			{ start_col = 0, end_col = #ctx.padded, hl_group = hl_group },
		}
	end
	...
	if col.key == "repo" then
		local hl_group = highlights.dynamic_for(row.repo)
		return {
			{ start_col = 0, end_col = #ctx.padded, hl_group = hl_group },
		}
	end
